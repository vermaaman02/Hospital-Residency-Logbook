/**
 * @module FacultyAssessmentsClient
 * @description Client component for faculty to manage internal assessments
 * for their assigned batches. Uses inline create/edit (no popup dialogs),
 * batch & semester selection, publishAt scheduling, evaluate/reject submissions.
 */
...
interface InlineForm {
	...
	resourceLinks: string;
	attachments: string[];
	assignedFacultyId: string; // "none" or uuid
...
type SubmissionStatusFilter = "all" | "SUBMITTED" | "SIGNED" | "NEEDS_REVISION";

// ======================== COMPONENT ========================

export function FacultyAssessmentsClient({
	assessments,
	batches,
	students,
	faculty,
}: FacultyAssessmentsClientProps) {
	...
	const openEdit = useCallback((a: AssessmentRow) => {
		...
		setForm({
			...
			assignedFacultyId: a.assignedFaculty?.id ?? "none",
			...
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div>
					...
					<p className="text-sm text-hospital-text-secondary mt-1">
						Manage assessments for your assigned batches
					</p>
				</div>
			</div>
