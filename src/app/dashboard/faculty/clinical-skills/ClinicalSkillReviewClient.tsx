/**
 * @module ClinicalSkillReviewClient
 ...
 * Two-tab view for Adult/Pediatric skills.
 *
...
export interface ClinicalSkillSubmission {
	id: string;
	slNo: number;
	skillName: string;
...
interface ClinicalSkillReviewClientProps {
	adultSubmissions: ClinicalSkillSubmission[];
	pediatricSubmissions: ClinicalSkillSubmission[];
	role: "faculty" | "hod";
	autoReviewEnabled?: boolean;
}

type StatusFilter = "ALL" | "SUBMITTED" | "SIGNED" | "NEEDS_REVISION" | "DRAFT";
type SkillType = "adult" | "pediatric";

const PAGE_SIZE = 10;

// ======================== MAIN COMPONENT ========================

export function ClinicalSkillReviewClient({
	adultSubmissions,
	pediatricSubmissions,
	role,
	autoReviewEnabled,
}: ClinicalSkillReviewClientProps) {
	const router = useRouter();
	const [isPending, startTransition] = useTransition();

	const [currentTab, setCurrentTab] = useState<SkillType>("adult");
	...
	const [detailEntry, setDetailEntry] =
		useState<ClinicalSkillSubmission | null>(null);

	// Sign/Reject dialogs
	const [signEntry, setSignEntry] = useState<ClinicalSkillSubmission | null>(
		null,
	);
	...
	const submissions =
		currentTab === "adult" ? adultSubmissions : pediatricSubmissions;

	// Available batches
	const batches = useMemo(() => {
		const set = new Set<string>();
		[...adultSubmissions, ...pediatricSubmissions].forEach((s) => {
			if (s.user.batchRelation?.name) set.add(s.user.batchRelation.name);
		});
		return Array.from(set).sort();
	}, [adultSubmissions, pediatricSubmissions]);
	...
	function handleTabChange(tab: string) {
		setCurrentTab(tab as SkillType);
		setCurrentPage(1);
	...
	// ---- Actions ----
	const handleSign = useCallback((entry: ClinicalSkillSubmission) => {
		setSignEntry(entry);
		setSignRemark("");
	}, []);
	...
	function confirmSign() {
		...
		startTransition(async () => {
			try {
				await signClinicalSkill(
					currentTab,
					signEntry.id,
					signRemark || undefined,
				);
				toast.success(
	...
	function openReject(entry: ClinicalSkillSubmission) {
	...
	function confirmReject() {
		...
		startTransition(async () => {
			try {
				await rejectClinicalSkill(currentTab, rejectEntry.id, rejectRemark);
	...
	function handleBulkSign() {
		...
		startTransition(async () => {
			try {
				await bulkSignClinicalSkills(currentTab, ids);
				setSelectedIds(new Set());
	...
	function handleAutoReviewToggle(enabled: boolean) {
		...
		startTransition(async () => {
			try {
				await toggleAutoReview("clinicalSkills", enabled);
				toast.success(
					`Auto review ${enabled ? "enabled" : "disabled"} for Clinical Skills`,
				);
				router.refresh();
			} catch {
				setAutoReview(!enabled);
	...
	const handleExportPdf = useCallback(async () => {
		const { exportClinicalSkillReviewToPdf } =
			await import("@/lib/export/export-pdf");
		const label = currentTab === "adult" ? "Adult" : "Pediatric";
		await exportClinicalSkillReviewToPdf(buildExportData(), role, label);
	}, [buildExportData, role, currentTab]);

	const handleExportExcel = useCallback(async () => {
		const { exportClinicalSkillReviewToExcel } =
			await import("@/lib/export/export-excel");
		const label = currentTab === "adult" ? "Adult" : "Pediatric";
		exportClinicalSkillReviewToExcel(buildExportData(), role, label);
	}, [buildExportData, role, currentTab]);
	...
	return (
		<div className="space-y-6">
			...
			<div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-wrap">
				{role === "hod" && (
					<div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border bg-muted/30">
						...
						<label
							htmlFor="cs-auto-review"
							className="text-xs font-medium text-muted-foreground cursor-pointer"
						>
							Auto Review (Clinical Skills)
						</label>
						<Switch
							id="cs-auto-review"
							checked={autoReview}
			...
			<Tabs value={currentTab} onValueChange={handleTabChange}>
				<TabsList className="grid w-full max-w-md grid-cols-2 mb-4">
					<TabsTrigger value="adult">
						Adult ({adultSubmissions.length})
					</TabsTrigger>
					<TabsTrigger value="pediatric">
						Pediatric ({pediatricSubmissions.length})
					</TabsTrigger>
				</TabsList>
					...
					<CardContent className="p-4">
						<div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
							<div className="relative flex-1 w-full">
								...
								<Input
									placeholder="Search by student, skill, or diagnosis…"
									value={searchQuery}
				...
				{/* Table content (shared for both tabs since data comes from `filtered`) */}
				<Card className="mt-4">
							...
							<Table>
								<TableHeader>
									<TableRow className="bg-muted/50">
										...
										<TableHead className="w-36 font-bold">Confidence</TableHead>
									...
									{paginated.map((entry) => (
										...
										>
											...
											<TableCell onClick={(e) => e.stopPropagation()}>
												<Link
													href={`/dashboard/${role}/clinical-skills/student/${entry.user.id}`}
													className="group"
												>
									...
									{paginated.length === 0 && (
										<TableRow>
											...
											>
												...
												:	"No clinical skill submissions yet."}
											</TableCell>
										</TableRow>
									)}
								</TableBody>
					...
					{detailEntry && (
						<>
							...
							<div className="mt-6 space-y-6">
								...
								<DetailSection title="Clinical Skill" icon={Activity}>
									<DetailRow label="Skill" value={detailEntry.skillName} />
									<DetailRow
										label="Diagnosis"
										value={detailEntry.representativeDiagnosis || "—"}
									/>
									<DetailRow
										label="Confidence"
								...
								{detailEntry.facultyRemark && (
									<DetailSection title="Faculty Remark" icon={MessageSquare}>
										<div className="bg-amber-50/50 border border-amber-200/50 rounded-lg p-3 text-sm">
											{detailEntry.facultyRemark}
										</div>
									</DetailSection>
								)}
			...
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Sign Clinical Skill Entry</DialogTitle>
						<DialogDescription>
							Skill #{signEntry?.slNo} — {signEntry?.skillName} by{" "}
			...
			>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Skill #{rejectEntry?.slNo} — {rejectEntry?.skillName} by{" "}
