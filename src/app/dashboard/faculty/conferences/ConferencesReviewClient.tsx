/**
 * @module ConferencesReviewClient
 * @description Faculty/HOD review page for student conference participation submissions.
 ...
 * @see actions/conferences.ts — signConferenceEntry, rejectConferenceEntry, bulkSignConferenceEntries
 */
...
interface ConferencesReviewClientProps {
...
export function ConferencesReviewClient({
	submissions,
	role,
	autoReviewEnabled,
	...
	// Detail sheet
	const [detailEntry, setDetailEntry] = useState<ConferenceSubmission | null>(
		null,
	);

	// Reject dialog
	const [rejectEntry, setRejectEntry] = useState<ConferenceSubmission | null>(
		null,
	);
	const [rejectRemark, setRejectRemark] = useState("");

	// Sign dialog
	const [signEntry, setSignEntry] = useState<ConferenceSubmission | null>(null);
		...
		if (searchQuery.trim()) {
			...
			result = result.filter(
				(s) =>
					...
					(s.conferenceName ?? "").toLowerCase().includes(q) ||
					(s.conductedAt ?? "").toLowerCase().includes(q) ||
					(s.participationRole ?? "").toLowerCase().includes(q) ||
	...
	// ---- Actions ----
	const handleSign = useCallback((entry: ConferenceSubmission) => {
		setSignEntry(entry);
		setSignRemark("");
	}, []);
	...
	function confirmSign() {
		...
		startTransition(async () => {
			try {
				await signConferenceEntry(signEntry.id, signRemark || undefined);
				toast.success(
					`Signed: ${signEntry.conferenceName ?? "Entry"} (${signEntry.user.firstName})`,
				);
				setSignEntry(null);
				setDetailEntry(null);
				setSelectedIds((prev) => {
	...
	function openReject(entry: ConferenceSubmission) {
	...
	function confirmReject() {
		...
		startTransition(async () => {
			try {
				await rejectConferenceEntry(rejectEntry.id, rejectRemark);
	...
	function handleBulkSign() {
		...
		startTransition(async () => {
			try {
				await bulkSignConferenceEntries(ids);
				setSelectedIds(new Set());
	...
	function handleAutoReviewToggle(enabled: boolean) {
		...
		startTransition(async () => {
			try {
				await toggleAutoReview("conferences", enabled);
				toast.success(
					`Auto review ${enabled ? "enabled" : "disabled"} for Conferences`,
				);
				router.refresh();
			} catch {
				setAutoReview(!enabled);
	...
	const handleExportPdf = useCallback(async () => {
		...
		const data = buildExportData().map((e) => ({
			...
			date: e.date ?? null,
			patientName: e.conferenceName ?? "—",
			patientAge: null,
			patientSex: null,
			uhid: null,
			completeDiagnosis: e.conductedAt ?? "—",
			procedureDescription: e.participationRole ?? "—",
		...
		await exportProcedureLogReviewToPdf(data, role, "Conferences");
	...
	const handleExportExcel = useCallback(async () => {
		...
		exportProcedureLogReviewToExcel(data, role, "Conferences");
	}, [buildExportData, role]);
	...
	return (
		<div className="space-y-6">
			...
			<div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-wrap">
				...
				{role === "hod" && (
					<div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border bg-muted/30">
						...
						<label
							htmlFor="conf-auto-review"
							className="text-xs font-medium text-muted-foreground cursor-pointer"
						>
							Auto Review (Conferences)
						</label>
						<Switch
							id="conf-auto-review"
							checked={autoReview}
				...
				<CardContent className="p-4">
					<div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
						...
						<div className="relative flex-1 w-full">
							...
							<Input
								placeholder="Search by student, conference name, venue, role, or batch..."
								value={searchQuery}
						...
						<Table>
							<TableHeader>
								<TableRow className="bg-muted/50">
									...
									<TableHead className="w-28 font-bold">Role</TableHead>
								...
								{paginated.map((entry) => (
									...
									>
										...
										<TableCell onClick={(e) => e.stopPropagation()}>
											<Link
												href={`/dashboard/${role}/conferences/student/${entry.user.id}`}
												className="group"
											>
								...
								{paginated.length === 0 && (
									<TableRow>
										...
										>
											...
											:	"No conference participation submissions yet."}
										</TableCell>
									</TableRow>
								)}
							</TableBody>
					...
					{detailEntry && (
						<>
								...
								<div className="flex items-start justify-between gap-3">
									<SheetTitle className="flex items-center gap-2.5">
										<div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-hospital-primary/10">
											<Award className="h-4.5 w-4.5 text-hospital-primary" />
										</div>
										Conference #{detailEntry.slNo}
									</SheetTitle>
									<StatusBadge
							...
							<div className="mt-6 space-y-6">
								...
								{/* Conference Details */}
								<DetailSection title="Conference Details" icon={CalendarDays}>
									<DetailRow
										label="Date"
										value={
											detailEntry.date ?
								...
								{/* Conference Name */}
								<DetailSection title="Conference Name" icon={Award}>
									{detailEntry.conferenceName ?
								...
								{/* Participation Role */}
								<DetailSection title="Participation Role" icon={Briefcase}>
									{detailEntry.participationRole ?
										<div className="bg-muted/30 rounded-lg p-3 text-sm">
											{detailEntry.participationRole}
										</div>
									:	<p className="text-sm text-muted-foreground pl-6">—</p>}
			...
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Sign Conference Participation Entry</DialogTitle>
						<DialogDescription>
							Entry #{signEntry?.slNo} by {signEntry?.user.firstName}{" "}
							{signEntry?.user.lastName}
							{signEntry?.conferenceName ?
								` — ${signEntry.conferenceName.substring(0, 60)}${signEntry.conferenceName.length > 60 ? "..." : ""}`
							:	""}
						</DialogDescription>
					</DialogHeader>
