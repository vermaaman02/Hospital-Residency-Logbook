/**
 * @module ResearchActivitiesReviewClient
 * @description Faculty/HOD review page for student research and outreach activity submissions.
 ...
 * @see actions/research-activities.ts — signResearchEntry, rejectResearchEntry, bulkSignResearchEntries
 */
...
interface ResearchActivitiesReviewClientProps {
...
export function ResearchActivitiesReviewClient({
	submissions,
	role,
	autoReviewEnabled,
	...
	// Detail sheet
	const [detailEntry, setDetailEntry] = useState<ResearchSubmission | null>(
		null,
	);

	// Reject dialog
	const [rejectEntry, setRejectEntry] = useState<ResearchSubmission | null>(
		null,
	);
	const [rejectRemark, setRejectRemark] = useState("");

	// Sign dialog
	const [signEntry, setSignEntry] = useState<ResearchSubmission | null>(null);
	...
	// ---- Actions ----
	const handleSign = useCallback((entry: ResearchSubmission) => {
		setSignEntry(entry);
		setSignRemark("");
	}, []);
	...
	function confirmSign() {
		...
		startTransition(async () => {
			try {
				await signResearchEntry(signEntry.id, signRemark || undefined);
				toast.success(
					`Signed: ${signEntry.activity ?? "Entry"} (${signEntry.user.firstName})`,
				);
				setSignEntry(null);
				setDetailEntry(null);
				setSelectedIds((prev) => {
	...
	function openReject(entry: ResearchSubmission) {
	...
	function confirmReject() {
		...
		startTransition(async () => {
			try {
				await rejectResearchEntry(rejectEntry.id, rejectRemark);
	...
	function handleBulkSign() {
		...
		startTransition(async () => {
			try {
				await bulkSignResearchEntries(ids);
				setSelectedIds(new Set());
	...
	function handleAutoReviewToggle(enabled: boolean) {
		...
		startTransition(async () => {
			try {
				await toggleAutoReview("researchActivities", enabled);
				toast.success(
					`Auto review ${enabled ? "enabled" : "disabled"} for Research Activities`,
				);
				router.refresh();
			} catch {
				setAutoReview(!enabled);
	...
	const handleExportPdf = useCallback(async () => {
		...
		const data = buildExportData().map((e) => ({
			slNo: e.slNo,
			categoryLabel: "Research Activities",
			date: e.date ?? null,
			patientName: e.activity ?? "—",
			patientAge: null,
			patientSex: null,
			uhid: null,
		...
		await exportProcedureLogReviewToPdf(data, role, "Research Activities");
	...
	const handleExportExcel = useCallback(async () => {
		...
		exportProcedureLogReviewToExcel(data, role, "Research Activities");
	}, [buildExportData, role]);
	...
	return (
		<div className="space-y-6">
			...
			<div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-wrap">
				...
				{role === "hod" && (
					<div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border bg-muted/30">
						...
						<label
							htmlFor="research-auto-review"
							className="text-xs font-medium text-muted-foreground cursor-pointer"
						>
							Auto Review (Research)
						</label>
						<Switch
							id="research-auto-review"
							checked={autoReview}
				...
				<CardContent className="p-4">
					<div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
						...
						<div className="relative flex-1 w-full">
							...
							<Input
								placeholder="Search by student, activity, venue, role, or batch..."
								value={searchQuery}
						...
						<Table>
							<TableHeader>
								<TableRow className="bg-muted/50">
									...
									<TableHead className="min-w-44 font-bold">Activity</TableHead>
								...
								{paginated.map((entry) => (
									...
									>
										...
										<TableCell onClick={(e) => e.stopPropagation()}>
											<Link
												href={`/dashboard/${role}/research-activities/student/${entry.user.id}`}
												className="group"
											>
										...
										<TableCell className="text-sm max-w-52">
											{entry.activity ?
												<div className="line-clamp-2">{entry.activity}</div>
								...
								{paginated.length === 0 && (
									<TableRow>
										...
										>
											...
											:	"No research activity submissions yet."}
										</TableCell>
									</TableRow>
								)}
							</TableBody>
					...
					{detailEntry && (
						<>
							<SheetHeader className="pb-4 border-b">
								<div className="flex items-start justify-between gap-3">
									<SheetTitle className="flex items-center gap-2.5">
										...
										Research Activity #{detailEntry.slNo}
									</SheetTitle>
									<StatusBadge
							...
							<div className="mt-6 space-y-6">
								...
								{/* Activity */}
								<DetailSection title="Activity" icon={FlaskConical}>
									{detailEntry.activity ?
			...
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Sign Research Activity Entry</DialogTitle>
						<DialogDescription>
							Entry #{signEntry?.slNo} by {signEntry?.user.firstName}{" "}
							{signEntry?.user.lastName}
							{signEntry?.activity ?
								` — ${signEntry.activity.substring(0, 60)}${signEntry.activity.length > 60 ? "..." : ""}`
							:	""}
						</DialogDescription>
					</DialogHeader>
