/**
 * @module HodStudentConferencesPage
 * @description HOD page to view a specific student's conference participation entries.
 *
 * @see PG Logbook .md — "CONFERENCE AND OTHER ACADEMIC ACTIVITY PARTICIPATION"
 * @see actions/conferences.ts — getStudentConferences
 */
...
import { getStudentConferences } from "@/actions/conferences";
...
export default async function HodStudentConferencesPage({ params }: Props) {
	...
	const conferences = await getStudentConferences(studentId);

	const stats = {
		total: conferences.length,
		signed: conferences.filter((c) => c.status === "SIGNED").length,
		pending: conferences.filter((c) => c.status === "SUBMITTED").length,
		needsRevision: conferences.filter((c) => c.status === "NEEDS_REVISION")
			.length,
	};
	...
	return (
		<div className="space-y-6">
			<div className="flex items-center gap-3">
				<Link href="/dashboard/hod/conferences">
			...
			<PageHeader
				title={`${student.firstName} ${student.lastName}'s Conferences`}
							...
							<TableBody>
								...
								{conferences.length === 0 && (
									<TableRow>
										...
										>
											No conference entries logged yet.
										</TableCell>
									</TableRow>
								)}
							</TableBody>
