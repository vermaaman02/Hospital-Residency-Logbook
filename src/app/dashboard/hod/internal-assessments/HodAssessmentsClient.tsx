...
export function HodAssessmentsClient({
	...
	const handleSave = useCallback(() => {
		...
		const input: CreateAssessmentInput = {
			...
			assignedFacultyId:
				form.assignedFacultyId !== "none" ? form.assignedFacultyId : undefined,
		};
						...
						>
							...
							<CardContent className="space-y-4">
								...
								<div className="space-y-2 border-t pt-4 pb-2">
									...
									<p className="text-xs text-muted-foreground mt-1">
										Leave all unchecked to assign to the entire batch/semester.
									</p>
								</div>
											...
											<div className="space-y-1">
												{evaluatingSubmission.attachments.map((link, i) => (
													<a
														...
														target="_blank"
														rel="noopener noreferrer"
														className="inline-flex items-center gap-2 text-sm text-hospital-primary hover:underline bg-background border rounded px-2 py-1 w-fit"
													>
