/**
 * @module PatientLogReviewClient
 * @description Reusable Faculty/HOD review page for patient log submissions.
 * Used by Imaging (5 categories), Transport, Consent, Bad News modules.
...
export interface PatientLogSubmission {
	id: string;
	slNo: number;
	category?: string; // imaging category enum, null for transport/consent/badnews
	date: string | null;
...
interface PatientLogReviewClientProps {
	submissions: PatientLogSubmission[];
	role: "faculty" | "hod";
	moduleLabel: string; // e.g. "Imaging", "Transport", "Consent", "Bad News"
	moduleIcon?: React.ReactNode;
	autoReviewEnabled?: boolean;
	autoReviewKey?: AutoReviewCategory; // key for toggleAutoReview
	skillLevelLabels: Record<string, string>;
	categoryOptions?: CategoryOption[]; // if module has sub-categories (imaging)
	categoryLabels?: Record<string, string>;
	studentLinkPrefix: string; // e.g. "/dashboard/faculty/imaging/student"
...
export function PatientLogReviewClient({
	submissions,
	role,
	moduleLabel,
	autoReviewEnabled,
	...
	// Detail sheet
	const [detailEntry, setDetailEntry] = useState<PatientLogSubmission | null>(
		null,
	);

	// Sign/Reject dialogs
	const [signEntry, setSignEntry] = useState<PatientLogSubmission | null>(null);
	const [signRemark, setSignRemark] = useState("");
	const [rejectEntry, setRejectEntry] = useState<PatientLogSubmission | null>(
		null,
	);
	...
	const categoryFiltered = useMemo(() => {
		if (!hasCategories || categoryFilter === "ALL") return submissions;
		return submissions.filter((s) => s.category === categoryFilter);
	}, [submissions, categoryFilter, hasCategories]);
	...
	const catLabel = (val: string | undefined) => {
		if (!val || !categoryLabels) return "—";
		return categoryLabels[val] ?? val;
	};
	...
	// ---- Actions ----
	const handleSign = useCallback((entry: PatientLogSubmission) => {
		setSignEntry(entry);
		setSignRemark("");
	}, []);
	...
	function confirmSign() {
		...
		startTransition(async () => {
			try {
				await onSign(signEntry.id, signRemark || undefined);
				toast.success(
	...
	function openReject(entry: PatientLogSubmission) {
	...
	function confirmReject() {
		...
		startTransition(async () => {
			try {
				await onReject(rejectEntry.id, rejectRemark);
	...
	function handleAutoReviewToggle(enabled: boolean) {
		...
		startTransition(async () => {
			try {
				const { toggleAutoReview } = await import("@/actions/auto-review");
				await toggleAutoReview(autoReviewKey ?? "imagingLogs", enabled);
				toast.success(
					`Auto review ${enabled ? "enabled" : "disabled"} for ${moduleLabel}`,
				);
				router.refresh();
			} catch {
				setAutoReview(!enabled);
	...
	}, [buildExportData, role, categoryFilter, hasCategories]);
	...
	return (
		<div className="space-y-6">
			...
			<Card>
				<CardContent className="p-0 sm:p-6 overflow-x-auto">
					<div
						...
						style={{ minWidth: hasCategories ? "1400px" : "1250px" }}
					>
						<Table>
							<TableHeader>
							...
							<TableBody>
								{paginated.map((entry) => (
									...
									>
										...
										<TableCell onClick={(e) => e.stopPropagation()}>
											<div className="group">
												<div className="text-sm font-medium text-hospital-primary">
								...
								{paginated.length === 0 && (
									<TableRow>
										<TableCell
											colSpan={hasCategories ? 14 : 13}
											className="text-center py-10 text-muted-foreground"
										>
											{searchQuery || statusFilter !== "ALL" ?
												"No matching entries found."
											:	`No ${moduleLabel.toLowerCase()} log submissions yet.`}
										</TableCell>
									</TableRow>
								)}
							</TableBody>
					...
					{detailEntry && (
						<>
								...
								<div className="flex items-start justify-between gap-3">
									<SheetTitle className="flex items-center gap-2.5">
										<div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-hospital-primary/10">
											<ScanLine className="h-4.5 w-4.5 text-hospital-primary" />
										</div>
										{moduleLabel} #{detailEntry.slNo}
									</SheetTitle>
									<StatusBadge
							...
							<div className="mt-6 space-y-6">
								...
								<DetailSection title={`${moduleLabel} Details`} icon={Activity}>
									{hasCategories && detailEntry.category && (
										<DetailRow
											label="Category"
											value={catLabel(detailEntry.category)}
										/>
									)}
									<DetailRow
										label="Date"
			...
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Sign {moduleLabel} Entry</DialogTitle>
						<DialogDescription>
							{moduleLabel} #{signEntry?.slNo} by {signEntry?.user.firstName}{" "}
			...
			>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							{moduleLabel} #{rejectEntry?.slNo} by{" "}
