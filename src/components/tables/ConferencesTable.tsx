/**
 * @module ConferencesTable
 * @description Inline-editing table for Conference and Academic Activity Participation.
 * Columns: Sl.No, Date, Conference/Academic Activity, Conducted@, Participation Role.
...
export function ConferencesTable({
	...
	const stats = useMemo(() => {
		...
		const filled = entries.filter((e) => e.conferenceName).length;
	...
	// ---- Edit handlers ----
	function startEditing(entry: ConferenceEntry) {
	...
	function handleSubmit(entry: ConferenceEntry) {
		if (!entry.conferenceName || !entry.date) {
			toast.error("Conference name and date are required to submit");
			return;
		}
		startTransition(async () => {
			try {
	...
	function handleDelete(entry: ConferenceEntry) {
				...
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<CardDescription>
								Conference and other academic activity participation
							</CardDescription>
						</div>
					</div>
					<Button
											...
											<TableCell>
												{isEditing ?
													<Input
														...
														placeholder="e.g., EMCON 2024, Workshop"
														className="h-8"
													/>
