/**
 * @module LogbookReviewsTable
 * @description Inline-editing table for Periodic Logbook Faculty Review entries.
 * Columns: Sl.No, Review No. (SEM 1-6), Date, Description of Work Done, Role in Activity, Faculty Sign, Status.
 *
...
interface LogbookReviewsTableProps {
...
export function LogbookReviewsTable({
	entries,
	facultyList,
	onAddRow,
	onDeleteEntry,
	...
	// ---- Edit handlers ----
	function startEditing(entry: LogbookReviewEntry) {
		if (entry.status === "SIGNED" || entry.status === "SUBMITTED") return;
		setEditingId(entry.id);
		setForm({
			reviewNo: entry.reviewNo ?? "",
	...
	function handleSubmit(entry: LogbookReviewEntry) {
	...
	function handleDelete(entry: LogbookReviewEntry) {
						...
						<div>
							<CardTitle className="flex items-center gap-2">
								...
								<Badge variant="outline" className="ml-2 text-xs">
									{stats.filled} / 6 semesters
								</Badge>
							</CardTitle>
							<CardDescription>
								Semester-wise faculty review of resident logbook
							</CardDescription>
						</div>
					</div>
					<Button
					...
					<Table>
						<TableHeader>
							<TableRow className="bg-muted/50">
								...
								<TableHead className="w-28">Review No.</TableHead>
											...
											<TableCell>
												{isEditing ?
													<Textarea
														...
														placeholder="Describe review content..."
