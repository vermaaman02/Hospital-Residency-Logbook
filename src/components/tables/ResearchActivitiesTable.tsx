/**
 * @module ResearchActivitiesTable
 * @description Inline-editing table for Other Research/Team Building/Teaching &
 * Training/Community Outreach Activity.
 * Columns: Sl.No, Date, Activity, Conducted@, Participation Role.
...
interface ResearchActivitiesTableProps {
...
export function ResearchActivitiesTable({
	entries,
	facultyList,
	maxEntries,
	onAddRow,
	...
	// ---- Edit handlers ----
	function startEditing(entry: ResearchEntry) {
		...
		startTransition(async () => {
			try {
				await onUpdateEntry(entryId, {
					...
					participationRole: form.participationRole || null,
	...
	function handleSubmit(entry: ResearchEntry) {
		if (!entry.activity || !entry.date) {
			toast.error("Activity and date are required to submit");
			return;
		}
		startTransition(async () => {
			try {
	...
	function handleDelete(entry: ResearchEntry) {
	...
	return (
		<Card>
				...
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<CardDescription>
								Other Research / Team Building / Teaching & Training / Community
								Outreach
							</CardDescription>
						</div>
					</div>
			...
			<CardContent className="space-y-4">
					...
					<Table>
						<TableHeader>
							<TableRow className="bg-muted/50">
								...
								<TableHead className="min-w-50">Activity</TableHead>
										...
										<React.Fragment key={entry.id}>
												...
												{isEditing ?
													<Input
														...
														onChange={(e) =>
															setForm((f) => ({
																...f,
																participationRole: e.target.value,
															}))
														}
														placeholder="e.g., Organizer, Mentor"
														className="h-8"
													/>
												:	<span className="text-sm">
											...
											<TableCell className="text-center">
												<RevisionThreadButton
													...
													description={`Entry #${entry.slNo} - ${entry.activity || "Activity"}`}
													label="History"
												/>
											</TableCell>
				...
				{/* Faculty Remarks Display for entries with remarks */}
				{entries.some(
					(e) => e.facultyRemark && e.status === "NEEDS_REVISION",
				) && (
					<div className="space-y-2 mt-4">
						...
						{entries
							.filter((e) => e.facultyRemark && e.status === "NEEDS_REVISION")
							.map((e) => (
								<div
									key={e.id}
									className="p-2 bg-red-50/50 dark:bg-red-950/20 rounded border border-red-100 dark:border-red-900"
								>
									<span className="text-sm font-medium">Entry #{e.slNo}:</span>
									<span className="text-sm ml-2 text-red-700 dark:text-red-300">
										{e.facultyRemark}
									</span>
								</div>
							))}
