...
export const IMAGING_CATEGORIES: ImagingCategoryConfig[] = [
	{
		...
		label:
			"Ultrasound & Echocardiography Examination: Non-Trauma Adult / Pediatric",
		maxEntries: 60,
	},
	{
		code: "G2",
		...
		label:
			"Point of Care Ultrasonography Examination: Trauma: Adult / Pediatric",
		maxEntries: 50,
	},
	{
		code: "G3",
	...
	},
	{
		...
		label:
			"Imaging Analysis X-Ray/ CT Scans/ MRI Brain: Non-Trauma: Adult / Pediatric",
		maxEntries: 10,
	},
	{
		code: "G5",
...
/** Convert enum to URL slug: ULTRASOUND_ECHO_NON_TRAUMA → ultrasound-echo-non-trauma */
export function imagingEnumToSlug(enumValue: string): string {
	return enumValue.toLowerCase().replace(/_/g, "-");
}

/** Convert URL slug to enum: pocus-trauma → POCUS_TRAUMA */
export function imagingSlugToEnum(slug: string): string | undefined {
	const enumValue = slug.toUpperCase().replace(/-/g, "_");
	const found = IMAGING_CATEGORIES.find((c) => c.enumValue === enumValue);
	return found?.enumValue;
}

/** Get category config by slug */
export function getImagingBySlug(
	slug: string,
): ImagingCategoryConfig | undefined {
	const enumValue = imagingSlugToEnum(slug);
	if (!enumValue) return undefined;
	return IMAGING_CATEGORIES.find((c) => c.enumValue === enumValue);
}
