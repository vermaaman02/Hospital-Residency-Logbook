...
import {
	useTransportLogs,
	TransportLogEntry,
} from "@/lib/hooks/useTransportLogs";
...
const LOCATION_PRESETS = ["ICU", "ER", "Ambulance", "Ward", "OR", "Trauma Bay", "Inter-hospital"];
const SEX_OPTIONS = ["Male", "Female", "Other"];
const MAX_TRANSPORT_ENTRIES = 10;
...
export default function TransportScreen() {
	...
	const {
		...
		signEntry,
		rejectEntry,
	} = useTransportLogs({ mode: userRole });

	const [editingEntry, setEditingEntry] = useState<TransportLogEntry | null>(null);
	const [rejectionModalEntry, setRejectionModalEntry] = useState<TransportLogEntry | null>(null);
	...
	const handleAddRow = async () => {
		if (entries.length >= MAX_TRANSPORT_ENTRIES) {
			Alert.alert(
				"Capacity Reached",
				`All ${MAX_TRANSPORT_ENTRIES} entry rows for Transport of Critically Ill Patient have already been added to your logbook.`
			);
			return;
		}
		try {
			const newEntry = await addRow();
			refetchEntries();
			if (newEntry?.entry) {
				setEditingEntry(newEntry.entry);
			} else if (newEntry) {
	...
	const handleClearDraft = (entry: TransportLogEntry) => {
		Alert.alert(
			"Clear Draft Fields",
				...
				<View style={styles.subViewHeader}>
					<HStack align="center" justify="space-between">
						<HStack align="center" gap="2" style={{ flex: 1, minWidth: 0, marginRight: 8 }}>
							...
							<Ambulance size={24} color={Colors.accent} />
							<VStack style={{ flex: 1, minWidth: 0 }}>
								<Heading level={3} numberOfLines={1}>
									Transport Logs
								</Heading>
								<Text variant="muted" numberOfLines={1}>
									Transport of Critically Ill Patient ({entries.length}/{MAX_TRANSPORT_ENTRIES})
								</Text>
							</VStack>
						</HStack>
								...
								<VStack gap="2">
									<HStack align="center" justify="space-between">
										<VStack gap="1" style={{ flex: 1, minWidth: 0, marginRight: 8 }}>
											<Heading level={3} numberOfLines={1}>Transport Progress</Heading>
											<Text variant="muted">
										...
										<HStack align="center" gap="2" style={{ flexShrink: 0 }}>
											<Badge label={`${filledCount}/${MAX_TRANSPORT_ENTRIES}`} tone="accent" />
											{userRole === "student" && (
												<Button
													label="+ Add Row"
													variant="primary"
			...
			<VStack gap="2">
				...
				<HStack align="center" justify="space-between" style={{ width: "100%" }}>
					<HStack align="center" gap="2" style={{ flex: 1, minWidth: 0, marginRight: 8 }}>
						...
						<Heading level={4} numberOfLines={1} style={{ flex: 1, flexShrink: 1 }}>
							{entry.patientName || `Transport Log #${entry.slNo}`}
						</Heading>
					</HStack>
				...
				<VStack gap="1" style={styles.entryFieldsBox}>
					...
					{entry.procedureDescription && (
						<VStack gap="1">
							<Text variant="muted">Procedure Description:</Text>
...
function TransportLogEditModal({
	entry,
	facultyList,
	visible,
	onClose,
	...
	const [performedAtLocation, setPerformedAtLocation] = useState(entry.performedAtLocation || "ICU");
	...
	const handleSave = async () => {
		try {
			...
			Alert.alert("Save Error", e.message || "Failed to save transport log entry");
		} finally {
			setSaving(false);
		}
	};
				...
				<View style={styles.editModalContent}>
					...
					<HStack align="center" justify="space-between" style={styles.editModalHeader}>
						<VStack style={{ flex: 1, minWidth: 0, marginRight: 8 }}>
							<Text variant="muted">Edit Transport Log — Sl {entry.slNo}</Text>
							<Heading level={3} numberOfLines={1}>
					...
					<ScrollView style={{ maxHeight: 460 }} contentContainerStyle={{ gap: 16, paddingBottom: 10 }}>
						{/* 1. Date Picker */}
						<VStack gap="1">
							<Text variant="bodyStrong">Transport Date</Text>
						...
						<Input
							...
							placeholder="Describe transport procedure details..."
						...
						<VStack gap="2">
							<Input
								label="Location"
								placeholder="e.g. ICU, ER, Ambulance, Inter-hospital, Ward..."
