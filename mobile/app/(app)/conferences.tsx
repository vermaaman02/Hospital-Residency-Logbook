...
import {
	ActivityIndicator, Alert, FlatList, Modal, Pressable, ScrollView, StyleSheet, View,
} from "react-native";
...
import { useConferences, ConferenceEntry } from "@/lib/hooks/useConferences";
...
export default function ConferencesScreen() {
	...
	const { entries, isLoadingEntries, summary, faculty, addRow, isAddingRow, updateEntry, submitEntry, deleteEntry } = useConferences();

	const [editingEntry, setEditingEntry] = useState<ConferenceEntry | null>(null);
	const [showDatePicker, setShowDatePicker] = useState(false);
	const [formState, setFormState] = useState({ date: null as Date | null, conferenceName: "", conductedAt: "", participationRole: "", facultyId: null as string | null });
	const [showFacultyPicker, setShowFacultyPicker] = useState(false);
	const [isSaving, setIsSaving] = useState(false);

	const openEditModal = (entry: ConferenceEntry) => {
	...
	const handleSave = async () => {
		...
		try {
			await updateEntry(editingEntry.id, {
				...
				conferenceName: formState.conferenceName || null,
				conductedAt: formState.conductedAt || null,
				participationRole: formState.participationRole || null,
	...
	const handleSubmit = (entry: ConferenceEntry) => {
		if (!entry.conferenceName) { Alert.alert("Validation", "Conference Name is required."); return; }
	...
	const handleDelete = (entry: ConferenceEntry) => {
	...
	const renderEntry = ({ item }: { item: ConferenceEntry }) => {
				...
				<VStack gap="2">
					<HStack justify="space-between" align="center">
						<HStack gap="2" align="center">
							<View style={[styles.slBadge, { backgroundColor: Colors.accent }]}><Text style={styles.slText}>{item.slNo}</Text></View>
							<Text variant="muted">{formatDate(item.date)}</Text>
						</HStack>
						<Badge label={st.label} tone={st.tone} />
					</HStack>
					{item.conferenceName ? <Text style={styles.primaryField}>{item.conferenceName}</Text> : <Text variant="muted" style={{ fontStyle: "italic" }}>No conference name</Text>}
					...
					{item.facultyRemark && item.status === "NEEDS_REVISION" && (
						<View style={styles.remarkBanner}><Text style={styles.remarkText}>⚠️ {item.facultyRemark}</Text></View>
					)}
					{isEditable && (
		...
		<Screen bleed>
				...
				ListHeaderComponent={
					<View style={styles.header}>
						<HStack align="center" gap="3">
							<Pressable onPress={() => router.back()} hitSlop={8}><ArrowLeft size={24} color={Colors.foreground} /></Pressable>
							<VStack gap="1" style={{ flex: 1 }}><Heading level={2}>Conferences</Heading><Text variant="muted">Conference & Academic Participation</Text></VStack>
						</HStack>
						{summary && (
						...
						<HStack gap="2" style={{ marginTop: Spacing["3"] }}>
							...
							<ExportButton module="conferences" size="sm" />
						</HStack>
					</View>
				}
			...
			<Modal visible={!!editingEntry} animationType="slide" transparent>
				<View style={styles.modalOverlay}><View style={styles.modalContent}><ScrollView showsVerticalScrollIndicator={false}>
					<HStack justify="space-between" align="center" style={{ marginBottom: Spacing["4"] }}><Heading level={3}>Edit Conference Entry</Heading><Pressable onPress={() => setEditingEntry(null)} hitSlop={10}><X size={22} color={Colors.foreground} /></Pressable></HStack>
					...
					<Input placeholder="e.g. National EM Conference" value={formState.conferenceName} onChangeText={(v) => setFormState((p) => ({ ...p, conferenceName: v }))} />
					...
					<Input placeholder="e.g. Presenter, Attendee, Organizer" value={formState.participationRole} onChangeText={(v) => setFormState((p) => ({ ...p, participationRole: v }))} />
