...
import { ArrowLeft, Calendar as CalendarIcon, MapPin, Minus, X } from "lucide-react-native";
import { Badge, Button, Card, Heading, HStack, Input, Screen, Text, VStack } from "@/components/ui";
import { ExportButton } from "@/components/ui/ExportButton";
import { useResearchActivities, ResearchEntry } from "@/lib/hooks/useResearchActivities";
...
export default function ResearchActivitiesScreen() {
	const router = useRouter();
	const { entries, isLoadingEntries, summary, faculty, addRow, isAddingRow, updateEntry, submitEntry, deleteEntry } = useResearchActivities();
	const [editingEntry, setEditingEntry] = useState<ResearchEntry | null>(null);
	const [showDatePicker, setShowDatePicker] = useState(false);
	const [formState, setFormState] = useState({ date: null as Date | null, activity: "", conductedAt: "", participationRole: "", facultyId: null as string | null });
	const [showFacultyPicker, setShowFacultyPicker] = useState(false);
	const [isSaving, setIsSaving] = useState(false);

	const openEditModal = (entry: ResearchEntry) => {
		setEditingEntry(entry);
		setFormState({ date: entry.date ? new Date(entry.date) : null, activity: entry.activity || "", conductedAt: entry.conductedAt || "", participationRole: entry.participationRole || "", facultyId: entry.facultyId || null });
	};
	...
	const handleSave = async () => {
		...
		setIsSaving(true);
		try {
			await updateEntry(editingEntry.id, { date: formState.date?.toISOString() || null, activity: formState.activity || null, conductedAt: formState.conductedAt || null, participationRole: formState.participationRole || null, facultyId: formState.facultyId } as any);
			setEditingEntry(null);
		} catch (e: any) { Alert.alert("Error", e?.message || "Failed to save"); }
		finally { setIsSaving(false); }
	};

	const handleSubmit = (entry: ResearchEntry) => {
		if (!entry.activity) { Alert.alert("Validation", "Activity is required."); return; }
	...
	const renderEntry = ({ item }: { item: ResearchEntry }) => {
		...
		return (
			<Card style={styles.entryCard}><VStack gap="2">
				<HStack justify="space-between" align="center">
					<HStack gap="2" align="center"><View style={[styles.slBadge, { backgroundColor: Colors.accent }]}><Text style={styles.slText}>{item.slNo}</Text></View><Text variant="muted">{formatDate(item.date)}</Text></HStack>
					<Badge label={st.label} tone={st.tone} />
				</HStack>
				{item.activity ? <Text style={styles.primaryField}>{item.activity}</Text> : <Text variant="muted" style={{ fontStyle: "italic" }}>No activity</Text>}
				{item.conductedAt ? <HStack gap="1" align="center"><MapPin size={14} color={Colors.muted} /><Text variant="muted">{item.conductedAt}</Text></HStack> : null}
				{item.participationRole ? <Text variant="muted">Role: {item.participationRole}</Text> : null}
		...
		<Screen bleed>
			<FlatList
				ListHeaderComponent={<View style={styles.header}>
					<HStack align="center" gap="3"><Pressable onPress={() => router.back()} hitSlop={8}><ArrowLeft size={24} color={Colors.foreground} /></Pressable>
						<VStack gap="1" style={{ flex: 1 }}><Heading level={2}>Research & Outreach</Heading><Text variant="muted">Research / Team Building / Teaching & Training</Text></VStack>
					</HStack>
					...
					<HStack gap="2" style={{ marginTop: Spacing["3"] }}>
						...
						<ExportButton module="research-activities" size="sm" />
					</HStack>
				</View>}
			...
			<Modal visible={!!editingEntry} animationType="slide" transparent>
				<View style={styles.modalOverlay}><View style={styles.modalContent}><ScrollView showsVerticalScrollIndicator={false}>
					<HStack justify="space-between" align="center" style={{ marginBottom: Spacing["4"] }}><Heading level={3}>Edit Research Entry</Heading><Pressable onPress={() => setEditingEntry(null)} hitSlop={10}><X size={22} color={Colors.foreground} /></Pressable></HStack>
					...
					<Input placeholder="Research / Teaching / Outreach activity" value={formState.activity} onChangeText={(v) => setFormState((p) => ({ ...p, activity: v }))} />
					<Text style={styles.fieldLabel}>Conducted At</Text>
					<Input placeholder="Location / Institution" value={formState.conductedAt} onChangeText={(v) => setFormState((p) => ({ ...p, conductedAt: v }))} />
					<Text style={styles.fieldLabel}>Participation Role</Text>
					<Input placeholder="e.g. Researcher, Trainer, Volunteer" value={formState.participationRole} onChangeText={(v) => setFormState((p) => ({ ...p, participationRole: v }))} />
