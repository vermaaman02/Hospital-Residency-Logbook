...
import { useLogbookReviews, LogbookReviewEntry } from "@/lib/hooks/useLogbookReviews";
...
export default function LogbookReviewsScreen() {
	const router = useRouter();
	const { entries, isLoadingEntries, summary, faculty, addRow, isAddingRow, updateEntry, submitEntry, deleteEntry } = useLogbookReviews();
	const [editingEntry, setEditingEntry] = useState<LogbookReviewEntry | null>(null);
	const [showDatePicker, setShowDatePicker] = useState(false);
	const [formState, setFormState] = useState({ reviewNo: "", date: null as Date | null, description: "", roleInActivity: "", facultyId: null as string | null });
	const [showFacultyPicker, setShowFacultyPicker] = useState(false);
	const [isSaving, setIsSaving] = useState(false);

	const openEditModal = (entry: LogbookReviewEntry) => {
		setEditingEntry(entry);
		setFormState({ reviewNo: entry.reviewNo || "", date: entry.date ? new Date(entry.date) : null, description: entry.description || "", roleInActivity: entry.roleInActivity || "", facultyId: entry.facultyId || null });
	};
	...
	const handleSave = async () => {
		...
		setIsSaving(true);
		try {
			await updateEntry(editingEntry.id, { reviewNo: formState.reviewNo || null, date: formState.date?.toISOString() || null, description: formState.description || null, roleInActivity: formState.roleInActivity || null, facultyId: formState.facultyId } as any);
			setEditingEntry(null);
		} catch (e: any) { Alert.alert("Error", e?.message || "Failed to save"); }
		finally { setIsSaving(false); }
	};

	const handleSubmit = (entry: LogbookReviewEntry) => {
	...
	const handleDelete = (entry: LogbookReviewEntry) => {
	...
	const renderEntry = ({ item }: { item: LogbookReviewEntry }) => {
			...
			<Card style={styles.entryCard}><VStack gap="2">
				<HStack justify="space-between" align="center">
					<HStack gap="2" align="center">
						<View style={[styles.slBadge, { backgroundColor: "#6366F1" }]}><Text style={styles.slText}>{item.slNo}</Text></View>
						<Text variant="muted">{formatDate(item.date)}</Text>
						{item.reviewNo ? <Badge label={`Review #${item.reviewNo}`} tone="neutral" /> : null}
					</HStack>
		...
		<Screen bleed>
			<FlatList
				ListHeaderComponent={<View style={styles.header}>
					<HStack align="center" gap="3"><Pressable onPress={() => router.back()} hitSlop={8}><ArrowLeft size={24} color={Colors.foreground} /></Pressable>
						<VStack gap="1" style={{ flex: 1 }}><Heading level={2}>Logbook Reviews</Heading><Text variant="muted">Periodic Logbook Faculty Review</Text></VStack>
					</HStack>
					...
					<HStack gap="2" style={{ marginTop: Spacing["3"] }}>
						...
						<ExportButton module="logbook-reviews" size="sm" />
					</HStack>
				</View>}
			...
			<Modal visible={!!editingEntry} animationType="slide" transparent>
				<View style={styles.modalOverlay}><View style={styles.modalContent}><ScrollView showsVerticalScrollIndicator={false}>
					<HStack justify="space-between" align="center" style={{ marginBottom: Spacing["4"] }}><Heading level={3}>Edit Logbook Review</Heading><Pressable onPress={() => setEditingEntry(null)} hitSlop={10}><X size={22} color={Colors.foreground} /></Pressable></HStack>
					<Text style={styles.fieldLabel}>Review No</Text>
					<Input placeholder="e.g. 1, 2, 3" value={formState.reviewNo} onChangeText={(v) => setFormState((p) => ({ ...p, reviewNo: v }))} />
					...
					<Input placeholder="Describe the review comments" value={formState.description} onChangeText={(v) => setFormState((p) => ({ ...p, description: v }))} multiline numberOfLines={3} style={{ minHeight: 80, textAlignVertical: "top" }} />
					<Text style={styles.fieldLabel}>Role in Activity</Text>
					<Input placeholder="e.g. Reviewer, Reviewee" value={formState.roleInActivity} onChangeText={(v) => setFormState((p) => ({ ...p, roleInActivity: v }))} />
