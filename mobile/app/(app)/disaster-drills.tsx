...
import { useDisasterDrills, DisasterDrillEntry } from "@/lib/hooks/useDisasterDrills";
...
export default function DisasterDrillsScreen() {
	const router = useRouter();
	const { entries, isLoadingEntries, summary, faculty, addRow, isAddingRow, updateEntry, submitEntry, deleteEntry } = useDisasterDrills();
	const [editingEntry, setEditingEntry] = useState<DisasterDrillEntry | null>(null);
	...
	const openEditModal = (entry: DisasterDrillEntry) => {
	...
	const handleSubmit = (entry: DisasterDrillEntry) => {
	...
	const handleDelete = (entry: DisasterDrillEntry) => {
	...
	const renderEntry = ({ item }: { item: DisasterDrillEntry }) => {
		...
		return (
			<Card style={styles.entryCard}><VStack gap="2">
				<HStack justify="space-between" align="center">
					<HStack gap="2" align="center"><View style={[styles.slBadge, { backgroundColor: "#F59E0B" }]}><Text style={styles.slText}>{item.slNo}</Text></View><Text variant="muted">{formatDate(item.date)}</Text></HStack>
		...
		<Screen bleed>
			<FlatList
				ListHeaderComponent={<View style={styles.header}>
					<HStack align="center" gap="3"><Pressable onPress={() => router.back()} hitSlop={8}><ArrowLeft size={24} color={Colors.foreground} /></Pressable>
						<VStack gap="1" style={{ flex: 1 }}><Heading level={2}>Disaster Management</Heading><Text variant="muted">Disaster / Mass Casualty / Prehospital EM</Text></VStack>
					</HStack>
					...
					<HStack gap="2" style={{ marginTop: Spacing["3"] }}>
						...
						<ExportButton module="disaster-drills" size="sm" />
					</HStack>
				</View>}
			...
			<Modal visible={!!editingEntry} animationType="slide" transparent>
				<View style={styles.modalOverlay}><View style={styles.modalContent}><ScrollView showsVerticalScrollIndicator={false}>
					<HStack justify="space-between" align="center" style={{ marginBottom: Spacing["4"] }}><Heading level={3}>Edit Disaster Drill Entry</Heading><Pressable onPress={() => setEditingEntry(null)} hitSlop={10}><X size={22} color={Colors.foreground} /></Pressable></HStack>
					...
					<Input placeholder="Describe the drill or activity" value={formState.description} onChangeText={(v) => setFormState((p) => ({ ...p, description: v }))} multiline numberOfLines={3} style={{ minHeight: 80, textAlignVertical: "top" }} />
					<Text style={styles.fieldLabel}>Role in Activity</Text>
					<Input placeholder="e.g. Triage Officer, Team Leader" value={formState.roleInActivity} onChangeText={(v) => setFormState((p) => ({ ...p, roleInActivity: v }))} />
